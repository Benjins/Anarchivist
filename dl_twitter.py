...
minPositionReg = re.compile('data-min-position="([a-zA-Z0-9=\-_\\+]*)"')

def ParseOutMinPosition(text):
    try:
        return minPositionReg.search(text.split('<div class="stream-container',1)[1].split('>',1)[0]).group(1)
    except:
        return None
...
def DownloadTweets(user):
    ...
    while data is not None:
        ...
        minPosition = ParseOutMinPosition(text)

        print('Added %d tweets... (last %d) (min pos: "%s")' % (len(tweetIDs), tweetIDs[-1], minPosition))

        params['max_position'] = minPosition
        url = '/search?' + urlencode(params)
        data = DownloadURL(url)
